...
6. Frontend CSS/module debt cleanup
   ...
   - Current state: chat viewport stability lives in `web/static/chat-layout.css`;
     ...
     timers live in `web/static/views/cultivation.js`. The resource stats modal
     and coverage renderer live in `web/static/views/resource_stats.js`; the
     world report modal lives in `web/static/views/world_report.js`. The

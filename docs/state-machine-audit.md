...
| Failure/manual fallback | `state.owners[].status` is `missing`, `stale`, `estimated`, or `fresh`. Anything except `fresh` tells the user to run `.储物袋` before relying on transfer numbers. |
...
| Refresh path | `/api/identities/modules` returns module state plus `status_summary`; official schedules can ask modules for an auto anchor. |
...
| Trigger | Message ingest classifies dungeon messages. `/api/dungeons/status` derives summaries from recent cards and hydrates context from open-room history when needed. |
| Refresh path | Recent view uses a fast window; expanded view does full lookup and refreshes the durable dungeon cache. |
| Failure/manual fallback | `.加入副本` is only a request until a bot success/failure reply arrives. Route advice is conservative and must surface uncertainty instead of pretending to know future outcomes. |
| Current gap | Cangkun has stronger route history than Xutian. Xutian still needs richer page-game presentation of phase, choice, route, and known loss boundaries. |
| Next action | Build dedicated dungeon panels on top of the existing guide/status payloads rather than inventing a second dungeon state store. |

...
| Refresh path | `/api/identity-state` returns module state plus `status_summary`; identity loading and manual refresh both refresh module state, and official schedules can ask modules for an auto anchor. Sidebar identity list, identity snapshot, sidebar module chips, and add-identity modal renderers are isolated in `web/static/views/identity_management.js`, while `web/static/app.js` keeps Telegram account/send_as API binding, global timer orchestration, and event orchestration. |

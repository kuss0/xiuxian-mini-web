...
| Refresh path | Recent view uses a fast window; expanded view does full lookup and refreshes the durable dungeon cache. Standalone Xutian/Cangkun guide modals are isolated in `web/static/views/xutian_guide.js` and `web/static/views/cangkun_guide.js`, with guide loading injected from `web/static/app.js`. |

...
| Refresh path | `/api/health`, `/api/health/audit`, and the health modal expose listener state without SSH. Account login modal and listen-target renderers are isolated in `web/static/views/account_management.js`, while `web/static/app.js` keeps account save/login/dialog/topic/listener API orchestration. |

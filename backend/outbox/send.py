        ...
        *,
        ...
        if send_as_peer is not None or (topic_id > 0 and reply_to_msg_id > 0):
            ...
            if topic_id > 0 and reply_to_msg_id > 0:
                reply_to_spec = input_reply_to_message(
                    reply_to_msg_id=int(reply_to_msg_id),
                    top_msg_id=int(topic_id),
                )
            elif reply_to_msg_id > 0:
                reply_to_spec = input_reply_to_message(reply_to_msg_id=int(reply_to_msg_id))
            elif topic_id > 0:
                reply_to_spec = input_reply_to_message(reply_to_msg_id=int(topic_id))
            result = await client(

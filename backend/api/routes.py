...
def build_get_routes(handlers: Mapping[str, Callable]) -> dict[str, Callable]:
    return {
        ...
        "/api/outbox/automation": handlers["_get_outbox_automation"],
...
def build_post_routes(handlers: Mapping[str, Callable]) -> dict[str, PostRoute]:
    return {
        ...
        "/api/outbox/auto-queue": PostRoute(handlers["_post_outbox_auto_queue"], needs_payload=True),
        "/api/outbox/automation/tick": PostRoute(handlers["_post_outbox_automation_tick"], needs_payload=True),
